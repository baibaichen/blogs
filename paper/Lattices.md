...
> 这个名字来源于数学：[lattice](https://en.wikipedia.org/wiki/Lattice_(order)) 是一个[偏序集](https://en.wikipedia.org/wiki/Partially_ordered_set)，**其中任何两个元素都有唯一的最大下界和最小上界**。
...
> **（a） **lattice声明了一些非常有用的主键和外键约束，**（b）**它帮助查询规划器将用户查询映射到 filter-join聚合物化视图（DW查询最有用的物化视图），**（c）**为Calcite提供了一个框架，在该框架内可以收集有关数据量和用户查询的统计信息，**（d）**允许 *Calcite* 自动设计和填充物化视图。
...
> 但有人可能想知道，5 件以下的订单是否比 100 件以上的订单更有利可图。突然，`单位销售额`成了一个维度。如果将列声明为维度列的成本几乎为零，那么我想让它们都成为维度列。
...
> **tile 是 lattice 中具有特定维度的物化表**。lattice JSON 元素的 tiles 属性定义了初始要物化的 tile s集。
...
View-based query rewriting aims to take an input query which can be answered using a pre-existing view and rewrite the query to make use of the view. Currently Calcite has two implementations of view-based query rewriting.
