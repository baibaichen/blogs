...
水位是“在**什么时候**（处理时间维度上）进行计算？”的答案的前半部分，它是事件时间维度上，输入完整性的时间概念。换句话说，水位是系统衡量管道进度和输入完整性的方法 ，它基于事件流中正被处理记录的事件时间（有穷或无穷数据源皆可，尽管对无穷数据源的用处更明显）。
...
<img src="102-figure-5.png" style="zoom:50%" alt="图5. 事件时间进度，偏差和水位" />*图5. 事件时间进度，偏差和水位*
