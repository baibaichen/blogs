...
See [Determining region count and size](http://hbase.apache.org/book.html#ops.capacity.regions) for configuration guidelines.
...
![region states](http://hbase.apache.org/images/region_states.png)
...
> For more information about date tiered compaction, please refer to the design specification at [https://docs.google.com/document/d/1_AmlNb2N8Us1xICsTeGDLKIqL6T-oHoRLZ323MG_uy8](https://docs.google.com/document/d/1_AmlNb2N8Us1xICsTeGDLKIqL6T-oHoRLZ323MG_uy8) 
...
Stripe compaction is fully compatible with [Compaction](http://hbase.apache.org/book.html#compaction) and works in conjunction with either the ExploringCompactionPolicy or RatioBasedCompactionPolicy. It can be enabled for existing tables, and the table will continue to operate normally if it is disabled later.
...
2. Configure other options if needed. See [Configuring Stripe Compaction](http://hbase.apache.org/book.html#ops.stripe.config) for more information.
...
### 71.3. Bulk Load Architecture

The HBase bulk load process consists of two main steps.

#### 71.3.1. Preparing data via a MapReduce job
...
#### 71.3.2. Completing the data load
...
For more information about the referenced utilities, see [ImportTsv](http://hbase.apache.org/book.html#importtsv) and [CompleteBulkLoad](http://hbase.apache.org/book.html#completebulkload).

See [How-to: Use HBase Bulk Loading, and Why](http://blog.cloudera.com/blog/2013/09/how-to-use-hbase-bulk-loading-and-why/) for a recent blog on current state of bulk loading.
...
See the Hadoop documentation on [HDFS Architecture](http://hadoop.apache.org/docs/current/hadoop-project-dist/hadoop-hdfs/HdfsDesign.html) for more information.
...
## 73. Timeline-consistent High Available Reads
...
![Timeline Consistency](http://hbase.apache.org/images/timeline_consistency.png)
...
### 73.9. Secondary replica failover
...
### 73.10. Configuration properties
...
#### 73.10.1. Server side properties
...
#### 73.10.2. Client side properties
...
### 73.12. Creating a table with region replication
...
### 74.3. Configuring the MOB Cache
...
### 74.4. MOB Optimization Tasks

#### 74.4.1. Manually Compacting MOB Files

To manually compact MOB files, rather than waiting for the [configuration](http://hbase.apache.org/book.html#mob.cache.configure) to trigger compaction, use the`compact_mob` or `major_compact_mob` HBase shell commands. These commands require the first argument to be the table name, and take an optional column family as the second argument. If the column family is omitted, all MOB-enabled column families are compacted.
