...
**Fence键**还扩展了本地在线验证技术[80]。传统系统中，可以在根到叶导航期间，仅针对兄弟节点而不是堂兄弟节点来验证先相邻的指针，因为可以从共享父节点中的信息中得知兄弟节点的身份，但是堂兄弟的指针验证需要I/O操作获取堂兄弟的父节点（也是第二个堂兄的祖父节点等）。因此，无论每种形式的验证有多少搜索操作，早期的技术[80]都无法验证B-tree中所有的正确性约束。另一方面，从叶节点到祖先节点，**fence键**沿着整个B树接缝是相等的，其中键值用作分隔符键。在一个B-tree中，可以利用一个**fence键**的值在每一级进行在线验证，在查询和更新处理过程中，B-tree中一个普通的根到叶下降，不仅可以验证具有共享父代的兄弟节点，还可以验证第一个堂兄弟节点、第二个堂兄弟节点等。即使叶节点是堂兄弟节点，搜索相邻叶节点两个键的操作验证了B-tree所有的约束，并且触摸所有叶子节点的搜索操作验证了整个B-tree中所有的正确性约束。

例如，图6.10中所示的索引中，两个根到叶的搜索可能以叶节点d和e结束。假设这两个根到叶搜索发生在单独的事务中。这两个搜索可以验证整个接缝的**fence键**是否正确。如图6.9所示，在使用相邻指针而不是**fence键**的**B树中，相同的两个根到叶搜索可以验证叶节点d和e中的条目确实比根节点中的分隔键小和大，但无法验证堂兄弟节点之间的指针是否相互一致。

通过提取和匹配事实进行验证不仅适用于传统的B-tree，也适用于**B^link^-tree**及其过渡状态。在节点拆分之后，父节点还没有在**B^link^-tree**中更新，因此生成上述事实。新分配的页面是普通页面，也会生成上述事实。最近拆分的页面是唯一具有特殊信息的页面，即邻居指针。因此，一个指示最近的分裂的节点，带有邻居指针，切尚未反映在父节点中，它必须触发生成一些特殊事实。由于这个旧节点为新节点提供了对应的“父节点事实”，如果想要延续父节点、子节点、祖先等的隐喻，那么旧节点可以称为“养父节点”。

> ==TODO：== Fig.6.11

图6.11说明了这种情况，其中节点*b*最近被拆分。关于节点*d*的**fence键**下界的事实不能从（将来的）父节点*a*导出。关于节点*d*的**fence键**下界的事实不能从（将来的）父节点*a*导出。因此，节点*b*不仅在搜索逻辑方面，而且在B-tree结构的验证期间，充当新节点*d*的临时父节点。 注意，图6.11中的中间状态也可以在删除B-tree节点时使用，同样可以在任何时间、任何节点序列中，以及因此在任何媒介上，执行完整和正确的B-tree验证。

除了验证B-tree结构之外，在提取事实之前必须验证每个单独的页面，并且可能需要相互匹配多个B-tree索引，例如，二级索引中对应主索引的标识符。页内验证非常简单，但可能会令人惊讶的是有多少细节值得验证。相互匹配多个索引非常类似于一个关联操作，可以使用所有标准的连接算法。或者，可以使用位图，如果位图指示存在错误，其中两个错误彼此屏蔽的概率微小，如果位图指示存在错误则需要第二次遍历。
