...
**搜索键中有重复的键值很平常**。重复的记录则不太常见，但在某些数据库中的确存在，即，如果不定义主键，则会在关系和表中存在混乱。对于重复的记录，标准的解决方案是要么存储多份拷贝，要么是储一份拷贝再加上一个计数器。前一个方法实现起来要简单一些，后一种方法则需要在查询操作中维护计数器，例如：在求和或是求平均值时要用到乘法、消除重复记录后要将计数器置为1、在**join**两个计数器时要相乘。

经由**非聚集索引**跳转到**聚集索引**，或是删除某条记录的B-tree索引时，重复的键值会导致歧义，所以B-tree索引中有重复键值并不是好事。因此，正如我们前面讨论的那样，B-tree的所有索引都必须唯一。尽管如此，仍可利用搜索键值前导字段中重复的键值，来减少存储空间，及提高搜索性能。

保存**非唯一键值**及其关联信息最直接的方法，是为每一个键值加上代表唯一信息的字段数组。非聚集索引中非唯一键的经典格式是加上记录标识作为键值，为了高效搜索，例如在删除时，可以使用记录标识的有序是列表。还可以利用一些简单的压缩方法，比如只存储相邻键值间记录标识不同的那部分，而不是保存完整的记录标识，就能最小化需要存储的字节。

这个方法一个更复杂的变体是前缀键值只保存一次，并将剩余的记录存储在数组中。例如，如果键值的前导字段只有少量的不同，区分键值的字段虽然有许多不同的值，但所需的空间很小，那么通过这个方法（键值的前导字段只保存一次），就可以节省存储空间。

另外一种表示**非唯一非聚集索引**的方法是使用**位图**，有多种形式和变体。这些将在下面讨论。

> ==TODO：==Fig 3.9

图3.9中的每行表示的都是相同的信息：**(a)**重复的键值分由不同的索引记录表示，每条索引记录（即每个键值）都附加上不同的记录标识符。**(b)**保存一份重复的键值及其对应的记录标识符数组。**(c)**前面两个方法的组合，适合于分割超长的记录标识符数组，比如记录标识符数组跨越了好几页。**(d)**和**(e)**展示了两个简单的压缩算法，基于截取共享前缀和行程编码，**“(9)2”**说明这条索引记录和其前面记录的前九个字符串相同，即，“Smith,471”，而“4711(2)”则表明连续两条条目以4711开始。(f)展示的是位图索引可能使用的表示法。通常会使用行程编码的某些变体来压缩位图索引。毫无疑问,读者们能自行设计出在此基础上额外变化的压缩算法，以及将不同的表示法组合起来。
...
扩充一下这些方法，就可用于实现『*Key*由多列组成』的B-Tree。例如，按第一个字段分组，那么『第一个字段中每一个不同的值』就可能会对应一组第二个字段，和一组详细信息。关系数据库中的**学生**和**课程**两张表，因为是多对多的关系，所以必定有一张保存这些多对多关系的表，这张表的第一个外键有许多不同的学生ID，每一个ID有一组该表的第二个外键（课程ID），以及一组描述这些关系的属性。信息检索中的某个全文索引，可能有许多不同的**关键字**，每个**关键字**对应的是一组包含这个**关键字**的**文档**，每个**文档**记录对应了一组在该**文档**中该**关键字**出现的位置。如果不考虑压缩，许多全文索引基本上就是这个格式。

如何“表示”重复键值是一方面，重复键值还牵涉到关系数据库的完整性约束，为了保证**唯一约束**，常利用B-Tree来防止插入重复键值。另一个不常用的技术，利用已经存在的B-Tree（假设存在合适的B-Tree）索引，来快速创建和验证新定义的**唯一约束**。为插入新键值而查找恰当的插入位置时，<u>同时返回和“未来”任意一个相邻键值的『最长共享前缀』，这和**动态前缀截取**的逻辑相似。根据共享前缀的长度，B-Tree索引中的元数据可能包含了针对不同“前缀”的一个计数器</u>。在『*Key*由多列组成』的B-Tree中，为每列维护一个计数器。一旦声明了唯一约束，这些计数器马上就能判断出是否违反了新声明的唯一约束。
...
A segment size with 215bit positions ensures that the bitmap for any segment easily fits into a database page; a segment size with 230bit positions ensures that standard integer values can be used in compression by run-length encoding. Dividing bitmaps into segments of 215or 230bit positions also enables reasonably efficient updates. For example, insertion of a single record require decompression and re-compression of only a single bitmap segment, and space management very similar to changing the length of a traditional B-tree record.
